//
//  PromptBoxCancelHeadViewController.m
//  injoy_XJPDC
//
...
@interface PromptBoxCancelHeadViewController ()
...
@implementation PromptBoxCancelHeadViewController
...
-(void)loadSubView{
    ...
    CGFloat fixContnetH = DefaultH-BottomH-20;
    ...
    if (fixContnetH > KBcontentH+10) {
        ...
        BGH = BottomH+contentH;
    }
    
    self.contentView.frame = CGRectMake(23,20,bgW-23*2, contentH);
    ...
    if (self.option == PromptBoxCancelHeadButtonNumberOne) {
...
-(void)letfButton:(UIButton *)sender{
    [self dismissView];
    if ([self.delegate respondsToSelector:@selector(selectCancelHeadLeftButton)]) {
        [self.delegate selectCancelHeadLeftButton];
    }
}

-(void)rightButton:(UIButton *)sender{
    [self dismissView];
    if ([self.delegate respondsToSelector:@selector(selectCancelHeadRightButton)]) {
        [self.delegate selectCancelHeadRightButton];
    }
