...
//    [scrollView addSubview:newUserRegis];
    ...
    [verCode addTarget:self action:@selector(verCodeButtonClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)loginButtonClick:(UIButton *)sender{
    ...
    if ([self.telTextField.text isEqualToString:@"123456"] && [self.pwTextField.text isEqualToString:@"123456"]) {
        ...
        YMAlertView *alertView = [[YMAlertView alloc] initWithTitle:@"登录失败" message:@"请输入正确的账号密码,请输入正确的账号密码请输入正确的账号密码,请输入正确的账号密码"];
        [alertView show];
    }

}
- (void)verCodeButtonClick{
    YMLoginViewController *loginViewController = [[YMLoginViewController alloc] init];
